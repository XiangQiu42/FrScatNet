...
from ...scattering2d.core.fr_scattering2d import fr_scattering2d
from ...frontend.torch_frontend import ScatteringTorch


class FrScatteringTorch2D(ScatteringTorch, ScatteringBase2D):
    def __init__(self, J, shape, L=8, max_order=2, pre_pad=False,
            backend='torch', out_type='array', alpha_1=1, alpha_2=1):
        ScatteringTorch.__init__(self)

        # 为了使用ScatteringBase2D构造函数，我们需要人为地去掉添加的两个分数阶参数，
        para_dic = {}
        keys = locals().keys()
        ...
        ScatteringBase2D.__init__(**para_dic)
        ...
        # add the two fractional parameter
        cot_1 = 0
        cot_2 = 0
        # calculate the cot(theta), where theta is the two fractional parameter,
        # default is 0
        if alpha_2 % 2 == 0 or alpha_1 % 2 == 0:
            raise ZeroDivisionError('alpha can\'t be even since sin(x) should NOT be zero!')
        else:
            if alpha_1 != 1:
                cot_1 = math.cos(alpha_1 * math.pi / 2) / math.sin(alpha_1 * math.pi / 2)
            if alpha_2 != 1:
                cot_2 = math.cos(alpha_2 * math.pi / 2) / math.sin(alpha_2 * math.pi / 2)
    ...
    def scattering(self, input):
        ...
        # At here, we try to use our fractional scatNet
        S = fr_scattering2d(input, self.pad, self.unpad, self.backend, self.J,
                            self.L, phi, psi, self.max_order, self.out_type,
                            cot_1=self.cot_1, cot_2=self.cot_2)
...
FrScatteringTorch2D._document()


__all__ = ['FrScatteringTorch2D']
